...
//  console.log(actuaTemp);
 ...
 weatherIcon.style.height='100px';
  ...
  //for map
  co_ordinates.textContent= 'Latitude: '+ data.coord.lat +' , Longitude: ' + data.coord.lon;
}
}).catch(err=>{
    console.log('srr' + err);
})
 ...
 let longitude;
  if (navigator.geolocation) {
    navigator.geolocation.getCurrentPosition(function showPosition(position) {
      ...
      co_ordinates.innerHTML = "Latitude: " + position.coords.latitude +
      ...
      /*const map = new google.maps.Map(document.getElementById("map")), {
       ...
       weatherUrl = `http://api.openweathermap.org/data/2.5/find?lat=${lat}&lon=${lon}&cnt=1&appid=a475908923b60eeeafd3fd4ce054d90e`;
        ...
        // locationMap.setCenter(center);
         weatherIcon.setAttribute=('src',`http://openweathermap.org/img/wn/${data.list[0].wind.icon}@2x.png`);
        }).catch(err=>{
            console.log(err);
        })
      });
